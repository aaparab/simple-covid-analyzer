...
def slice_fit(df, state='Illinois', polydeg=6):
    ...
    _df['_fitvals'] = _fitvals
    
    return _df

def plot(casedf=None, deathdf=None, fit=True, state='Illinois', saveloc=None):
    """Plots dataset."""
    ...
    plt.suptitle('COVID-19 cases in {} per day'.format(state))
    ...
    ax.plot(df1.index, df1._diff, 'g.', label=ylabel1)
    if fit is True:
        ax.plot(df1.index, df1._fitvals, 'g')
    ax.grid()
    ax.legend()
    ...
    if nrows == 2:
        ...
        ax.plot(df2.index, df2._diff, 'b.', label=ylabel2)
        if fit is True:
            ax.plot(df2.index, df2._fitvals, 'b')
        ax.grid()
        ax.legend()
